...
func Command() *cobra.Command {
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("error installing azure nextgen plugin: %v", err)
			}
	...
	f.StringVarP(&region, "region", "r", "UsWest", "Azure region to deploy to")
	...
	_, err = containerservice.NewManagedCluster(ctx, fmt.Sprintf("%s-aksctl-cluster", name), &containerservice.ManagedClusterArgs{
		...
		AgentPoolProfiles: containerservice.ManagedClusterAgentPoolProfileArray{
			&containerservice.ManagedClusterAgentPoolProfileArgs{
				Name:         pulumi.String(fmt.Sprintf("%s-default", name)),
